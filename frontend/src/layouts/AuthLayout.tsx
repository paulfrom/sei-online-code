...
import { Link, history as umiHistory, useIntl } from 'umi';
...
const AuthLayout: React.FC = () => {
  ...
  const getMenuNavItemByMode = (item: any) => {
    const label = resolveRouteTitle(item.title);
    return <Link to={item.path}> {label} </Link>;
  };
  ...
  const getMenuItems = (data: any[]) => {
    return data.map((item) => {
      const menuTitle = resolveRouteTitle(item.title);
      if (!menuTitle) {
        return undefined;
      }
      ...
      return (
        <Menu.Item title={menuTitle} key={item.path}>
