...
- 新建项目工作区成功后自动初始化项目记忆。
- 平台 seed 记忆模板内容可配置，新项目统一使用当前 active default 模板。
- 对已有项目，补齐项目记忆模板并初始化平台记忆。
...
```text
1. DB 中 ACTIVE 且 is_default=true 的 MemorySeedTemplate
2. classpath 内置 default seed
```

第一版不做多模板选择，新项目统一使用当前 active default seed。后续如果需要按项目类型选择模板，可在 Project 创建流程增加 seedTemplateCode 选择。
...
第一版只内置 `default` seed，并在初始化时导入或同步到 DB 的默认模板记录。
...
- 发布新版本时，旧 active 默认模板改 `ARCHIVED`，新版本改 `ACTIVE`。
- 新项目初始化只读取 active default 模板。
...
- 从 `MemorySeedTemplateService` 读取 active default seed。
...
1. 项目创建。
2. 工作区创建/克隆/脚手架完成。
3. afterCommit 从平台 default seed 创建 agent-memory 初始模板。
4. afterCommit 投递 MEMORY_INITIALIZE job。
5. 后台扫描 agent-memory、项目规范、代码现状。
...
- `publish` 发布新版本，并将其设为 active default。
- `archive` 只能归档非当前 active default，或先发布新默认模板后再归档旧模板。
...
6. 修改 Requirement / OverviewDesign / DetailedDesign，增加 design_context_id 和 validation 字段。
7. 新增平台 default seed 记忆模板资源。
8. 实现 MemorySeedTemplateService，支持 bootstrap、draft、publish、archive。
9. 实现 AgentMemoryTemplateService，从 active default seed 创建 agent-memory 文件。
...
4. 创建 oc_memory_job。
5. 为 oc_requirement 增加 design_context_id / memory_validation_status / memory_validation_result_json。
6. 为 oc_overview_design 增加 design_context_id / memory_validation_status / memory_validation_result_json。
7. 为详细设计表增加 design_context_id / memory_validation_status / memory_validation_result_json。
8. oc_workspace_memory 包含 memory_spec_version、memory_seed_template_id 和 agent_memory_seed_version。
9. 创建索引。
10. 创建唯一约束或通过 service 保证每个 project/requirement 一个 CURRENT。
```

注意：

- 现有历史 PRD、概览设计、详细设计的 `design_context_id` 允许为空。
- `memory_validation_status` 对历史数据默认 `NOT_RUN`。
- confirm 拦截需兼容历史数据，建议仅对新生成或存在 memory 功能标记的文档强制执行；具体兼容策略在实现前再确认。

## 22. 关键兼容策略

### 22.1 历史数据

历史需求没有 `design_context_id`。

第一版建议：

```text
历史文档可查看。
历史文档如果要重新生成，必须走新 context。
历史文档如果要确认，若 memory_validation_status 为空或 NOT_RUN，需要先校验或提示用户迁移。
```

实现前需确认是否允许旧文档 bypass memory validation。
...
```text
从 active default seed 自动补齐缺失模板。
不覆盖已有文件。
...
如果要求 platform-memory 写入失败阻断 CURRENT，需要在实现前另行确认。当前建议不阻断 DB 主流程。
...
- [ ] seed 模板支持 draft / publish / archive。
