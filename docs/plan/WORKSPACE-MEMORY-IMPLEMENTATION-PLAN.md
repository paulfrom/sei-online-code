...
### 8.2 `oc_requirement_design_context`
...
### 10.1 `AgentMemoryTemplateService`

职责：

- 创建 `agent-memory` 四个空模板。
- 新建项目工作区成功后自动创建。
- 已存在项目接入时缺文件补齐。
- 不覆盖已有文件。
- 不把平台推断结论写入 `agent-memory`。

### 10.2 `WorkspaceMemoryService`
...
### 10.3 `WorkspaceMemoryScannerService`
...
### 10.4 `RequirementDesignContextService`
...
### 10.6 `PlatformMemoryWriterService`
...
### 10.7 `DesignContextPromptAssembler`
...
### 10.8 `DesignMemoryValidationService`
...
1. 项目创建。
2. 工作区创建/克隆/脚手架完成。
3. afterCommit 创建 agent-memory 空模板。
...
3. 新增 DTO / DAO。
4. 新增 V24__memory_foundation.sql。
5. 修改 Requirement / OverviewDesign / DetailedDesign，增加 design_context_id 和 validation 字段。
6. 实现 AgentMemoryTemplateService。
7. 实现 PlatformMemoryWriterService 的基础写入能力。
8. 实现 MemoryJobService 的投递、查询、重试、状态流转。
9. 接入项目工作区 ready 后的 MEMORY_INITIALIZE 投递点。
10. 新增基础 Memory API 和 Controller。
```
...
```text
1. 创建 oc_workspace_memory。
2. 创建 oc_requirement_design_context。
3. 创建 oc_memory_job。
4. 为 oc_requirement 增加 design_context_id / memory_validation_status / memory_validation_result_json。
5. 为 oc_overview_design 增加 design_context_id / memory_validation_status / memory_validation_result_json。
6. 为详细设计表增加 design_context_id / memory_validation_status / memory_validation_result_json。
7. 创建索引。
8. 创建唯一约束或通过 service 保证每个 project/requirement 一个 CURRENT。
```
