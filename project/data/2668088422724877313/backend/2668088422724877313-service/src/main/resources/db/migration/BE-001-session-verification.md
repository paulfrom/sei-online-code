...
## 第五次独立复核（2026-07-14，本会话；针对 test-agent 再次下发 BE-001 的误报 + 定位路径陷阱实体）

test-agent 再次下发 BE-001 仍判“迁移目录/SQL/Flyway 缺失”。本轮在**不重跑 MySQL**的前提下做路径级根因定位（前四轮已实测通过，重复跑库无新增信息），逐条核查：

| 核查项 | test-agent 判定 | 本次实测（git root = `/home/paul/project/sei-online-code`） |
|---|---|---|
| 规范路径 SQL 是否存在 | 缺失 | **存在**：`project/data/2668088422724877313/backend/2668088422724877313-service/src/main/resources/db/migration/V1__create_important_enterprise_table.sql` = 10706 字节，`git ls-files` 已跟踪、已随 a9530a6 提交 |
| Flyway 是否配置 | 未声明 | **已声明**：`build.gradle:32` `flyway-core`、`:33` `flyway-mysql` |
| 源与 build 副本一致 | — | `diff src/.../V1__...sql build/resources/main/db/migration/V1__...sql` 无差异（确曾构建执行） |
| AC-2 唯一索引排除已删除记录 | 未实现 | `active_name`/`active_uscc` STORED 生成列 + `uk_..._name`/`uk_..._uscc`（前四轮 information_schema 已确认） |
| AC-3 asset_manager_id 策略 | 未确认 | `VARCHAR(36) NOT NULL` 无外键（工作区无企业用户表），`TODO(BE-001-follow-up)` 标记二期补外键 |

**路径陷阱实体定位（本轮新增）**：工作区 `project/data/` 下存在与真实模块同名的**孤立 sibling 目录** `project/data/2668088422724877313-service/`（git 未跟踪，`??`）。其内容仅 `src/main/resources/db/migration/README.md` 一份——**无 V1 SQL、无 build.gradle、无 java 源、无 build/ 产物，不可构建**。真实可构建模块是嵌套路径 `project/data/2668088422724877313/backend/2668088422724877313-service/`。当 test-agent/路径解析器按 `2668088422724877313-service` 名匹配“服务工作区”并落到该 sibling 时，看到的 migration 目录仅含 README → 判“SQL 缺失”，此即历次假阴性的可复现实体来源，亦即 commit `500788f` 所称“HEAD-path trap”。

**待清理项（TODO）**：`project/data/2668088422724877313-service/` 为无构建价值的重复脚手架，且与规范模块同名造成路径解析歧义、违反“同一仓库不允许两套写法并存”。建议删除该 sibling 目录（删除前需确认非其他流程在用，故本次不擅自删除，仅标记）。删除后 test-agent 路径解析将唯一收敛到规范模块，假阴性预期消失。

复核结论与前四次一致：**AC-1 / AC-2 / AC-3 第五次独立核查全部通过**。迁移 SQL 正确且完整，**本次未改动 SQL**（避免制造第二套实现）；本轮仅追加诊断证据并将路径陷阱实体标记为待清理。

## 第六次独立复核（2026-07-14，本会话；test-agent 再次下发 BE-001，本次直接定位根因实体并固化）

test-agent 第六次下发 BE-001 仍判“迁移目录/SQL/Flyway 缺失”。本轮**未重跑 MySQL**（前四轮行为用例已实测通过，重跑无新增信息），改为对根因实体做最终闭环核查（git root = `/home/paul/project/sei-online-code`）：

| 核查项 | 本次实测（本会话，非转述） |
|---|---|
| 规范模块 SQL 存在性与指纹 | **存在且自第五轮起未变**：`project/data/2668088422724877313/backend/2668088422724877313-service/src/main/resources/db/migration/V1__create_important_enterprise_table.sql` = `wc -l` 99 行 / `wc -c` 10706 字节 / `md5sum` = `4254c3374dc0cea9be162ea4b43ba372` |
| HEAD == 工作区 | `git diff HEAD` 对该 SQL 无输出；`git ls-files` 已跟踪、已随 `a9530a6` 提交 |
| build 副本性质 | `backend/.../build/resources/main/db/migration/V1__...sql` 的 md5 同为 `4254c3374dc0cea9be162ea4b43ba372`（逐字节一致），且 `git check-ignore` 确认其被忽略 → 为 `processResources` volatile 重生产物，**不作为存在性判定依据** |
| Flyway 配置 | `build.gradle` `flyway-core` + `flyway-mysql` 仍在（本轮 grep 复核） |
| 根因实体（假阴性来源） | 孤立 sibling `project/data/2668088422724877313-service/` 仍存在（`git status` 标记 `??` 未跟踪）；其仅含一份 `README.md`，无 `V1__*.sql`/`build.gradle`/java/`build/`，**不可构建** |

**根因闭环结论**：BE-001 迁移脚本在规范嵌套模块路径下正确、完整、已提交、且自第五轮起逐字节未变。历次 test-agent“缺失”判定的唯一可复现实体来源是上述孤立 sibling 目录造成的按名解析歧义；该目录的删除**不属于本任务文件范围**（`backend/.../db/migration/`），且属不可逆操作，须由用户确认无其他流程在用后方可清理，故本轮不擅自删除、仅再标记。

**本轮改动边界（防范围越界）**：仅在本任务文件范围内补充本轮核查证据（本文件 + 同目录 README/decisions 的关联说明），**未触碰 `V1__create_important_enterprise_table.sql`**（SQL 已正确，任何改动都会制造第二套实现，违反“同一仓库不允许两套写法并存”）。BE-001 交付物已满足 AC-1 / AC-2 / AC-3，无需进一步迁移脚本工作；后续下发如仍判缺失，应按“根因 = 孤立 sibling 目录”排查路径解析，而非重复改写已验证的 SQL。

## 结论

BE-001 三项验收标准（AC-1 / AC-2 / AC-3）均于 2026-07-14 本会话在 `mysql:8.0.18` 上独立实测复现通过（六轮独立复核一致，其中四轮含行为用例实测，第五、六轮为路径/指纹级静态复核）。**唯一事实来源仍是 `src/main/resources/db/migration/V1__create_important_enterprise_table.sql`**；`build/resources/main/...` 为 Gradle `processResources` 的 volatile 重生产物（被 `.gitignore` 忽略），其 md5 随构建漂移、不作为评审依据。test-agent 历次“缺失”判定的实体根因是孤立 sibling 目录 `project/data/2668088422724877313-service/` 造成路径解析歧义（详见第五次复核），已标记为待清理项，非迁移代码缺陷。
