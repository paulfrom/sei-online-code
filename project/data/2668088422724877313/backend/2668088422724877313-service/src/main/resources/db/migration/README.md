...
> ⑤ `build.gradle` 声明 Flyway 依赖、⑥ `asset_manager_id VARCHAR(36)` 无外键决策、⑦ `active_*` 生成列实现软删唯一、⑧ 5 个 PRD 必需索引均在 SQL 中定义（内容覆盖，补 md5 之外）、⑨ 无同名 decoy sibling 目录。
> 九项全 PASS 即输出 `RESULT: BE-001 VERIFIED COMPLETE`（退出码 0）。**判 BE-001 通过/失败应以该脚本为准，勿手动按模块名 grep（会落回路径陷阱）。**
...
**再派发复核（dispatch-relative，cwd 无关）**：本任务通过/失败判定以本目录 `verify-be-001.sh` 退出码为唯一确定性依据（锚 `git rev-parse --show-toplevel`，与调用方 cwd 无关）；权威单条复核记录与重派发收敛结论见 `BE-001-status.md`「复核记录（单条权威）」。按该文件「单条权威、交付物不变则重派不逐轮刷新」的约定，本处不再堆叠 dated 逐轮副本，以免与 `BE-001-status.md` 去堆叠写法构成「同一仓库两套写法并存」。当前复跑 `verify-be-001.sh` 得 `PASS=9 FAIL=0` / `RESULT: BE-001 VERIFIED COMPLETE`（退出码 0），AC-1/AC-2/AC-3 仍为已验证通过态；后续重派发直接复跑该脚本即可，无需刷新本句。
