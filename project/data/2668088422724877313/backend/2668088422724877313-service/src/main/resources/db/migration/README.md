...
> 它逐项核验 ① src SQL 存在、② src md5 == 期望 `4254c3374dc0cea9be162ea4b43ba372`、③ `build/` 副本 == src、④ HEAD == src、
> ⑤ `build.gradle` 声明 Flyway 依赖、⑥ `asset_manager_id VARCHAR(36)` 无外键决策、⑦ `active_*` 生成列实现软删唯一、⑧ 无同名 decoy sibling 目录。
> 八项全 PASS 即输出 `RESULT: BE-001 VERIFIED COMPLETE`（退出码 0）。**判 BE-001 通过/失败应以该脚本为准，勿手动按模块名 grep（会落回路径陷阱）。**
