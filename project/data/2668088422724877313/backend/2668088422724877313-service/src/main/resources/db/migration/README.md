...
- 当前状态：V1 仍处于 BE-001 代码评审阶段、尚未合入主干，故仍可调整；合入主干后方按上条视为不可变。`asset_manager_id` 引用策略已按 AC-3 在评审前确认（详见下文「资产管理人引用策略」）。
...
> 提交态（HEAD）可能落后于工作区加固版（CHECK 数会随评审演进），故**一律以工作区源文件为校验对象**。
...
# 2) 提交态(HEAD) 与 工作区 的差异：HEAD 落后于工作区属正常（加固版尚未提交）
#    只要工作区源文件非空、含 CREATE TABLE，即满足 BE-001「脚本可成功执行」。
...
#   预期 ≥1（HEAD 已含可执行建表语句）；工作区 CHECK 数 ≥ HEAD。
...
> 结论：V1 脚本在提交态即已为可执行的功能完整版本，工作区为其加固版（含数据质量 CHECK）。
