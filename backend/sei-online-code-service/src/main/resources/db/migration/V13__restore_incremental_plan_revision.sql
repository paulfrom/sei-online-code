-- Restore the incremental plan revision migration that was accidentally removed.
--
-- The statements are idempotent because some environments already received part
-- of this schema through Hibernate DDL update. Explicit ALTER COLUMN statements
-- repair defaults on those pre-existing columns.
...
ALTER TABLE oc_requirement
    ALTER COLUMN revision_seq SET DEFAULT 0,
    ALTER COLUMN applied_revision_seq SET DEFAULT 0,
    ALTER COLUMN revision_state SET DEFAULT 'NONE';
